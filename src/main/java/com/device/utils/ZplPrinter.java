...
/**
 * 采用点阵字库ts24.lib 根据ZPL指令
 * 
 * @author sxf 20180420
 */
public class ZplPrinter {
	private String printerURI = null; // 打印机完整路径 统一资源标识符
	 ...
	 *            打印机路径
	 */
	public ZplPrinter(String printerURI,String path) throws FileNotFoundException {
		this.printerURI = printerURI;
		boolean flag = getts24lib(path);
		// 判断ts24lib是否存在
		if (!flag) {
			return;
		}
		...
		AppContext.getAppContext().put(
				PrintServiceLookup.class.getDeclaredClasses()[0], null);// 刷新打印机列表
		// 获取本机打印机服务
		...
		if (printService == null) {
			...
			System.out.println("打印机名称：["
					+ printService.getAttribute(PrinterName.class).getValue()
					+ "]");
		}
	}
	 ...
	 * 
	 */
	public boolean getts24lib(String path) throws FileNotFoundException {
		boolean flag = true;
		// 加载字体
		// 24点阵字体资源包路径 //需要下载
		// File file = new File("C://ts24.lib");/label_dev/src/file/ts24.lib
		...
		if (file.exists()) {
			...
			System.out.println(path+ " ts24.lib文件未找到");
			flag = false;
		}
		return flag;
	}
	...
	public void setBarcode(String barcode, String zpl) {
		content += zpl.replaceAll("$data", barcode);// jdk 1.8
		// content += zpl.replace("${data}", barcode);//jdk1.7
	}

	// 打印二维码指令
	public void setCode(String str, int x, int y, int h, int w) {
		content += "^FO" + x + "," + y + "^BQ," + h + "," + w + "^FDLA" + str
				+ "^FS";
	}
	 ...
	 *            中文字体间距。24是个比较合适的值。
	 */
	public void setText(String str, int x, int y, int eh, int ew, int es,
			int mx, int my, int ms) {
		// 字符串转字节数组
	...
	public void setCharR(String str, int x, int y, int h, int w) {
		content += "^FO" + x + "," + y + "^A0R," + h + "," + w + "^FD" + str
				+ "^FS";
	}

	/**
	 * 获取完整的ZPL 拼接 begin content end
	 * 
	 * @return
	 */
